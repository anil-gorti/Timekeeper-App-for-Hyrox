...
import { Calendar, MapPin, UserPlus, WifiOff, Volume2, VolumeX, Maximize } from "lucide-react";
...
export const StatusBar = memo(function StatusBar({ onNewAthlete, onSpectatorMode }: StatusBarProps) {
  const { athleteName, athleteBib, raceElapsedMs, totalTransitionMs, eventConfig, allComplete } = useRace();
  ...
  return (
    <div className="w-full">
      <AppHeader
        ...
        centerLabel={eventConfig.eventName || "HYFIT GAMES 2.1"}
      />

      {/* Side sheet opened by header menu */}
      ...
      {/* Mid strip + progress are hidden on results screen to avoid duplication */}
      {!allComplete && (
        <>
          <div className="h-16 w-full flex items-center justify-between px-4 sm:px-6 bg-black border-b border-[#1A1A1A] relative">
            <div className="flex items-center gap-3">
              <div>
                <div className="text-[12px] text-gray-500 uppercase tracking-wider font-medium mb-0.5">
                  Race time
                </div>
            ...
            <div className="flex flex-col items-center justify-center absolute left-1/2 -translate-x-1/2">
              ...
              <div className="text-[#CCFF00]/90 text-sm font-mono mt-0.5 tabular-nums">
                #{athleteBib || "—"}
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 z-10">
              <div className="text-right w-24">
                <div className="text-[11px] text-gray-500 uppercase tracking-wider font-medium mb-0.5">
                  Transition
                </div>
                <div className="text-base font-mono text-gray-400 tabular-nums">
                  {formatRaceTime(totalTransitionMs)}
                </div>
              </div>
            </div>
          </div>
          ...
          {!isOnline && (
            <div className="flex items-center justify-center gap-1.5 py-1 bg-red-900/40 border-b border-red-800/50">
              ...
              <span className="text-red-400 text-[10px] font-semibold tracking-wider uppercase">
                Offline
              </span>
            </div>
          )}
        ...
        <DialogContent className="bg-[#111] border-[#333] text-white">
          ...
          <div className="space-y-4 py-2">
            <Input
              ...
              onKeyDown={async (e) => {
                if (e.key === "Enter") {
                  const ok = await setAdminWithPin(adminPin);
                  if (ok) {
          ...
          <DialogFooter className="gap-2">
            <Button
              ...
              className="border-[#444] text-gray-400 text-sm"
            >
              Cancel
            </Button>
            <Button
              size="sm"
              className="bg-[#CCFF00] text-black hover:bg-[#aacc00] text-sm font-semibold"
              onClick={async () => {
                const ok = await setAdminWithPin(adminPin);
                if (ok) {
                  setAdminPinDialogOpen(false);
